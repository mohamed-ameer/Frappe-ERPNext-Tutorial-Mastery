...
// Strips comments and collapses whitespace so sprites ship as few bytes as possible
...
function writeSprite(filename, symbolTags) {
    ...
    const raw = `<svg id="frappe-symbols" aria-hidden="true" style="position: absolute; width: 0; height: 0; overflow: hidden;" xmlns="http://www.w3.org/2000/svg">\n${symbolTags.join('\n')}\n</svg>`;
    const content = minifySvgContent(raw);
...
// -------------------------------------------------------------
// Generate one sprite per package
// -------------------------------------------------------------
for (const source of sources) {
    buildSpriteFromDir(source);
}

if (fs.existsSync(iconifyDir)) {
    for (const setName of iconifySets) {
        buildIconifySprite(setName);
    }
}
